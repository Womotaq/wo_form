...
class _PickDatePageWithYearState extends State<PickDatePageWithYear> {
  ...
  void initState() {
    ...
    monthScrollController = InfinitePageController(
      initialIndex: yearMonthCenter.fullMonth._clamp(
        widget.minDate?.fullMonth,
        widget.maxDate?.fullMonth,
      ),
...
class _FullMonthCubit extends Cubit<int> {
  ...
  Future<void> setYear(int year) async {
    ...
    final fullMonth = _FullMonth.build(
      year: year,
      month: state.month,
    ...
    return BlocBuilder<_FullMonthCubit, int>(
      builder: (context, currentFullMonth) {
        return _SelectableIndex(
          ...
          onSelect: () => context.read<_FullMonthCubit>().setFullMonth(
            fullMonth,
            fromMonths: true,
          ),
        );
...
class MonthlyCalendar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final selectedDay = fullMonth == selectedDate?.fullMonth
        ? selectedDate!.day
        : null;
...
class InfinitePageController extends PageController {
  ...
  int processIndex(int index) => _processIndex(
    index.toDouble(),
    minIndex: minIndex,
    maxIndex: maxIndex,
  ).toInt();
